...
public final class AppLovinHandler: AdsHandler {
    ...
    public func loadBanner(
        ...
        let view = Banner(adUnitIdentifier: id, configuration: configs, placement: placement)
        switch size {
        case .standart, .medium, .large:
            break
        case .adaptive:
            if let size = view.adView.adFormat?.adaptiveSize {
                view.set(size: size)
            }
        case let .custom(width, height):
            view.set(size: CGSize(width: width, height: height))
        }
...
private extension AppLovinHandler {
    final class Banner: UIView, MAAdViewAdDelegate {
        var loadContinuation: CheckedContinuation<Banner, Error>?
        let adView: MAAdView
        
        @MainActor
        init(adUnitIdentifier: String, configuration: MAAdViewConfiguration?, placement: String?) {
            adView = MAAdView(adUnitIdentifier: adUnitIdentifier, configuration: configuration)
